...
class PlusSAINTModule(pl.LightningModule):
    def __init__(self):
        ...
        # Thêm biến tạm để tính AUC sau mỗi epoch
        self.train_outs = []
        self.train_labels = []
    ...
    def training_step(self, batch, batch_ids):
        ...
        self.train_outs.append(out.detach().cpu())
        self.train_labels.append(labels.detach().cpu())
        self.log("train_loss", loss, on_step=True, prog_bar=True)
        return loss

    def on_train_epoch_end(self):
        out = torch.cat(self.train_outs).numpy().reshape(-1)
        labels = torch.cat(self.train_labels).numpy().reshape(-1)
...
if __name__ == "__main__":
    ...
    trainer.fit(model=saint_plus, train_dataloaders=train_loader, val_dataloaders=val_loader)
